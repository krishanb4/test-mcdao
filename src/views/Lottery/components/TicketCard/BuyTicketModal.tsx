  ...
  return (
    <Modal title={t('Enter amount of tickets to buy')} onDismiss={onDismiss}>
      <TicketInput
        ...
        availableSymbol="MCDAO"
      />
      <div>
        <Tips>{t('1 Ticket = %lotteryPrice% MCDAO', { lotteryPrice: LOTTERY_TICKET_PRICE })}</Tips>
      </div>
      <div>
        <Announce>
          {t('Ticket purchases are final. Your MCDAO cannot be returned to you after buying tickets.')}
        </Announce>
        <Final>{t('You will spend: %num% MCDAO', { num: cakeCosts(val) })}</Final>
      </div>
      <ModalActions>
